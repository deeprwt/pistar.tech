...
interface ShopAreaProps {
  productId?: string; // Optional: used to show only one product
}

const ShopArea = ({ productId }: ShopAreaProps) => {
  const [filterableProducts, setFilterableProducts] = useState<IProduct[]>([]);

  // Filtering based on props (productId)
  useEffect(() => {
    if (productId) {
      const product = product_data.find((item) => item.id === Number(productId));
      setFilterableProducts(product ? [product] : []);
    } else {
      setFilterableProducts([...product_data]);
    }
  }, [productId]);
  ...
  const handleSelectChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    let updatedProducts = [...product_data];

    if (value === 'low') {
      updatedProducts = updatedProducts.sort((a, b) => Number(a.price) - Number(b.price));
    } else if (value === 'high') {
      updatedProducts = updatedProducts.sort((a, b) => Number(b.price) - Number(a.price));
    } else if (value === 'books') {
  ...
  return (
    <div className="product-section-one mt-150 lg-mt-80 mb-150 lg-mb-60">
      <div className="container">
        ...
        {/* Show filters only when showing all products */}
        {!productId && (
        ...
        <div className="products-wrapper mt-40">
          <div className="row gx-xxl-5">
            {currentItems.map((product, i) => (
              <div key={product.id} className="col-lg-4 col-sm-6 wow fadeInUp" data-wow-delay={`0.${i}s`}>
                <ProductItem product={product} />
              </div>
            ))}
          </div>

          {/* Show pagination only when not filtering by productId */}
          {!productId && (
